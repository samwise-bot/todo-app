...
describe('BoardLanesSection', () => {
  ...
  test('renders board-level and column-level empty states', () => {
    ...
    expect(html).toContain('class="btn btn-secondary lane-move-btn lane-move-btn-touch-target"');
    expect(html).toContain('class="board-toolbar-lanes-divider board-toolbar-lanes-divider-compact"');
  ...
  test('marks saved-view chip active when current board filters match preset', () => {
    ...
    expect(html).toContain('board-saved-view-row-active');
    expect(html).toContain('Active: <strong>Assigned to me</strong>');
    expect(html).toContain('One-click reset to all cards:');
